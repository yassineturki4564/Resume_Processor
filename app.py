...
def total_experience_from_intervals(intervals):
    events = []  # Will store start and end events

    for start, end in intervals:
        events.append((start, 1))  # 1 signifies start of an interval
        events.append((end, -1))  # -1 signifies end of an interval

    events.sort()  # Sorting by years
    ...
    for year, event_type in events:
        if prev_year is not None and active_intervals > 0:
            total_experience += (year - prev_year)
...
def extract_years_of_experience(experience_levels):
    ...
    return total_experience_from_intervals(intervals)
...
def get_download_link(filename, text):
    ...
    return href
def main():
    st.title('Resume Processor with URL Tracker Integration')

    # Part for URL integration
    uploaded_file = st.file_uploader("Upload the trackedDownloads.json (if available)", type="json")
    urls = []

    if uploaded_file:
        tracked_data = json.load(uploaded_file)
        urls = [tracked_data[key] for key in tracked_data]
    ...
    if uploaded_files:
        ...
        if st.button("Process"):
            ...
            df_simple['Original_Order'] = df_simple['PDF File'].apply(lambda x: uploaded_files_order.index(os.path.basename(x)))
            df_simple = df_simple.sort_values(by=['Original_Order'])
            if urls:
                df_simple['URLs'] = urls
            df_simple.drop(columns='Original_Order', inplace=True)
