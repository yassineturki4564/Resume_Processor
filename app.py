...
def main():
    ...
    if uploaded_files:
        ...
        if st.button("Process"):
            ...
            df_simple_simple = df_simple.drop(columns=['PDF File'])
            if df_simple_simple is not None and not df_simple_simple.empty:
                current_time_now = datetime.now().strftime("%Y/%m/%d_%H/%M/%S")
                sanitized_keywords = "_".join(keywords.split(";")).replace(" ", "_")
                output_file = f"{current_time_now}_{sanitized_keywords}_{str(uuid.uuid4())}.xlsx"
                df_simple_simple.to_excel(output_file, index=False, engine="openpyxl")
